...
export class SkyContribListViewGridComponent
  ...
  @ViewChild('chooseColumnsTemplate') public chooseColumnsTemplate: TemplateRef<any>;
  /* tslint:enable */

  @ContentChildren(SkyContribListViewGridColumnComponent)
  public columnComponents: QueryList<SkyContribListViewGridColumnComponent>;
    ...
    let sub = this.gridState.map(s => s.displayedColumns.items)
      ...
      .subscribe(displayedColumns => {
        ...
        this.dispatcher.sortSetAvailable(
          displayedColumns.filter(c => c.field !== undefined).map(cmp =>
  ...
  public openColumnSelector() {
    ...
    let modalInstance = this.modalService
      .open(SkyContribListViewGridColumnSelectorComponent, providers);
  ...
  public isSelected(id: string): Observable<boolean> {
    return this.state.map(s => {
      return s.selected.item[id] !== undefined;
    }).distinctUntilChanged();
  }
