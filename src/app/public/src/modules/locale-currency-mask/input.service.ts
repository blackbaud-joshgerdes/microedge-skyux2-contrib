...
export class InputService {
  ...
  constructor (
    ...
    this.inputManager = new InputManager(this.htmlInputElement);
  ...
  public addNumber(keyCode: number): void {
    if (!this.rawValue) {
  ...
  public applyMask(isNumber: boolean, rawValue: string): string {
    rawValue = isNumber ? Number(rawValue).toFixed(2) : rawValue;
  ...
  public clearMask(rawValue: string): number {
  ...
  public removeNumber(keyCode: number): void {
    let selectionEnd = this.inputSelection.selectionEnd;
    let selectionStart = this.inputSelection.selectionStart;
  ...
  public updateFieldValue(selectionStart?: number): void {
