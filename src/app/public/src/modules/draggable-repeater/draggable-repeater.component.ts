...
export class SkyContribDraggableRepeaterComponent implements OnDestroy {
  @Input() public dragOnHandle: boolean = false;
  @Output() public onItemsReordered = new EventEmitter<any>();
  @ContentChildren(SkyContribDraggableRepeaterItemComponent)
    public items: QueryList<SkyContribDraggableRepeaterItemComponent>;
  public bagName: string = this.generateGuid();
  ...
  constructor(private dragulaService: DragulaService) {
    ...
    this.subscriptions.push(this.dragulaService.drop.subscribe(([e, element, container]: any[]) => {
      let itemIds: any[] = [];
